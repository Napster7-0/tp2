...
export const MpmChart = ({ projectData }: MpmChartProps) => {
  ...
  const renderChart = () => {
    ...
    const calculateLevel = (
      ...
      return (
        1 +
        Math.max(
          ...task.predecessors.map((pred) => calculateLevel(pred, visited))
        )
      );
    };
    ...
    tasks.forEach((task) => {
      task.predecessors.forEach((predId) => {
        connections.push({
          from: predId,
          to: task.id,
          isCritical: task.isCritical && projectData[predId].isCritical,
        });
      });
    });
    ...
    const endTasks = tasks.filter(
      (task) => !tasks.some((t) => t.predecessors.includes(task.id))
    );
  ...
  return (
    <div
      ref={chartRef}
      className="w-full overflow-x-auto bg-gray-50 p-4 rounded-lg"
    />
  );
};
